...
export default function MapPage() {
  ...
  const hasHeading = Number.isFinite(userHeading)
  const normalizedHeading = useMemo(() => {
    if (!hasHeading) return 0
    const mod = userHeading % 360
...
const toggleFullScreen = () => {
  ...
  const mapRotationAngle = shouldRotateMap ? -normalizedHeading : 0
    ...
    <div className={`App ${isFullScreen ? "fullscreen-map" : ""}`}>
      ...
      <div className={`map-container ${isFullScreen ? "fullscreen" : ""}`}>
        <div
          ...
          style={{ transform: `rotate(${mapRotationAngle}deg)` }}
        >
              ...
              <CustomOverlayMap position={myLocation}>
                <div className={`user-location-marker${isGuiding ? " user-location-marker--guiding" : ""}`}>
                  <span
                    ...
                    style={{
                      transform: isGuiding
                        ? `translateY(-2px) rotate(${hasHeading ? normalizedHeading : 0}deg)`
                        : "translateY(-2px)",
                    }}
                  />
